...
public class RoomMaster : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (cameraReset == true)
        {
            ...
            mainCamera.transform.position = Vector3.Lerp(mainCamera.transform.position, playerPosition, 15f * Time.deltaTime);
            mainCamera.GetComponent<Camera>().orthographicSize = Mathf.Lerp(mainCamera.GetComponent<Camera>().orthographicSize, originalSize, 15f * Time.deltaTime);
